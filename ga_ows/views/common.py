    ...
    def GetValidTimes(self, r, kwargs):
        ...
        if 'callback' in parms.cleaned_data and parms.cleaned_data['callback']:
            return HttpResponse("{callback}({js})".format(
                ...
                js=json.dumps([t.strftime('%Y.%m.%d-%H:%M:%S.%f') for t in self.adapter.get_valid_times(**parms.cleaned_data)]), content_type='test/jsonp'
            ))
        else:
            return HttpResponse(json.dumps([t.strftime('%Y.%m.%d-%H:%M:%S.%f') for t in self.adapter.get_valid_times(**parms.cleaned_data)]), content_type='application/json')
...
class GetValidVersionsMixin(OWSMixinBase):
    ...
    def GetValidVersions(self, r, kwargs):
        ...
        if 'callback' in parms.cleaned_data and parms.cleaned_data['callback']:
            return HttpResponse("{callback}({js})".format(
                callback=kwargs['callback'],
                js=json.dumps([t for t in self.adapter.get_valid_versions(**parms.cleaned_data)]), content_type='test/jsonp'
            ))
        else:
            return HttpResponse(json.dumps([t for t in self.adapter.get_valid_versions(**parms.cleaned_data)]), content_type='application/json')
...
class GetValidElevationsMixin(OWSMixinBase):
    ...
    def GetValidElevations(self, r, kwargs):
        ...
        if 'callback' in parms.cleaned_data:
            return HttpResponse("{callback}({js})".format(
                callback=kwargs['callback'],
                js=json.dumps([t for t in self.adapter.get_valid_elevations(**parms.cleaned_data)]), content_type='test/jsonp'
            ))
        else:
            return HttpResponse(json.dumps([t for t in self.adapter.get_valid_elevations(**parms.cleaned_data)]), content_type='application/json')
...
class OWSView(View, GetCapabilitiesMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponse(ex.xml(extend=self.extended_exceptions), content_type='text/xml')
