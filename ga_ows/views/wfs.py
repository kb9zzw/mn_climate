...
class DescribeFeatureTypeMixin(WFSBase):
    ...
    def _response_json_DescribeFeatureType(self, response, callback=None):
        ...
        if callback is not None:
            return HttpResponse(callback + "("  + json.dumps(rsp) + ")", content_type='text/javascript')
        else:
...
class GetFeatureMixin(WFSBase):
    ...
    def GetFeature(self, request, kwargs):
        ...
        if 'gml' in output_format or 'xml' in output_format:
            ...
            responsef.close()
            os.unlink(tmpname)
            return HttpResponse(rdata, content_type=output_format)
            ...
            responsef.close()
            os.unlink(tmpname)
            return HttpResponse(rdata, content_type=mimetypes.get(output_format,'text/plain'))
        else:
...
class ListStoredQueriesMixin(WFSBase):
    ...
    def ListStoredQueries(self, request, kwargs):
        ...
        return HttpResponse(etree.tostring(response, pretty_print=True), content_type='text/xml')
